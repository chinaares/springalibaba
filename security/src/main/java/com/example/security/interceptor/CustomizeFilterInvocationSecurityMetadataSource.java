...
public class CustomizeFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object o) throws IllegalArgumentException {
        ...
        if (!requestUrl.contains("?")) {
            permissionList =  sysPermissionService.selectListByPath(requestUrl);
        } else {
            //为get请求的情况下，要切割字符串，因为?后面带的是一些参数，我们在数据库保存的仅仅是请求地址
            String [] arrs = requestUrl.split("\\?");
           permissionList =  sysPermissionService.selectListByPath(arrs[0]);
        }
        ...
        for(int i = 0;i<permissionList.size();i++){
            attributes[i] = permissionList.get(i).getKey();
        }
